...
class OffGridEvidenceContract(_ClosedContract):
    ...
    Disclosure follows the settled precedent rather than widening it: SEC-011 already permits
    object counts, ADR-0079 refusals already carry byte-offset locators, and a *routed* preview
    already publishes ``start_pad_id`` and ``end_pad_id`` for the same net. This says strictly
    less than a successful preview of the same request. It carries no board density: no object
    counts, no net names, no absolute coordinates -- only a relative miss and a divisor.
    """
