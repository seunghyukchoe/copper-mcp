...
| `preview_live_layered_route` | The layered proposal against one active KiCad snapshot, additionally bound to the redacted KiCad session revision. Candidate-only. | `expect_board_revision`, `expect_snapshot_digest`, `expect_session_revision` → `candidate_id`, `base_revision` | `COPPER_MCP_ALLOW_LIVE_IPC=1` |
...
| `obstacle_budget_exceeded` | route, layered route, routing job | The obstacle model needed more work than `max_obstacles` / `max_obstacle_checks` allows. | Same as above: reduce the problem before raising a ceiling. This is an admission that the work ran out, **not** a proof that no route exists. |
...
| `unsupported_board` | placement, apply | The board is outside the subset this operation admits. | Stop for this board. Report the limit rather than trying a different tool as a workaround. |
...
| `invalid_token` | apply | The `apply_token` does not verify against the in-process key, or its operation domain is wrong — a route token can never authorize a placement write, or the reverse. | Re-run the matching preview with `include_apply_token: true` and use the token it returns. Never reuse a token across operations or across boards. |
| `token_expired` | apply | The token verified but is past its lifetime. Tokens are signed with a key that exists only in the running process, so a server restart invalidates every outstanding token. | Re-preview to mint a fresh token, then apply promptly. Do not cache tokens across sessions. |
| `token_already_used` | apply | Single-use means single-use. | Re-preview. If you did not intend a second apply, **stop and report** — something already wrote to this board. |
