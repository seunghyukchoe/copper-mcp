...
| `preview_route_bundle` | One atomic plan over two to eight known net references, or nothing. Publishes only when negotiated routing, a complete composition replay, and the cross-net clearance gate all succeed. No partial plans, no token. Opt-in `include_drc` continues a routed plan through one authoritative KiCad DRC run over the composed board: single-invocation evidence bound to the bundle, never a reproducible differential and never apply authority. A DRC failure refuses the whole response; re-request without the flag for the plan alone. | `expect_board_revision`, `expect_snapshot_digest` → `bundle_id`, `base_revision`, `snapshot_digest` | `include_drc` |
...
| `invalid_candidate` | external route verification | The closed foreign document is structurally well formed but cannot be reconstructed as the coordinator-bound candidate it claims to propose. | Stop and regenerate the document from the same observed problem. Do not copy or forge identity fields; none are accepted. |
| `discontinuous_path` | external route verification | Consecutive foreign segments do not form an ordered continuous path. | Correct the external producer. CopperMCP does not repair or reorder the submission. |
| `endpoint_mismatch` | external route verification | The submitted route does not join the coordinator-selected endpoint components. | Rebuild against the exact pad refs and revisions. Do not edit endpoint IDs merely to silence the refusal. |
| `undeclared_layer` | external route verification | A submitted segment or via names a layer outside the coordinator-derived routing problem. | Regenerate within the declared layers. The verifier never widens the layer set from the document. |
| `infeasible` | external route verification | The completed structural proof found that the submitted patch cannot join all required components. | Submit a different complete patch. This is not a budget refusal and CopperMCP will not repair it. |
| `obstacle_violation` | external route verification | Exact bounded geometry proved submitted copper intersects an obstacle. | Reject the candidate and regenerate it. Do not relabel or apply it. |
| `budget_exceeded` | external route verification | A server-derived segment, edge, connectivity, or obstacle work ceiling stopped verification. | Reduce the foreign patch, or deliberately choose different bounded coordinator settings and re-observe both revisions. This code is not an infeasibility proof. |
| `deadline_exceeded` | external route verification | The absolute verification/DRC deadline expired before a publishable bound result existed. | Nothing was decided. Re-observe before a deliberate retry; never treat it as a clean or invalid route. |
...
| `not_run` | refused external route verification `physical_validation` | Structural intake refused before an authoritative DRC result could be published. | "DRC failed", "DRC passed", or any physical verdict. KiCad may not have started. |
| `completed` | accepted external route verification `physical_validation` | The authoritative KiCad command completed and candidate-bound evidence was published. | "passed", "clean", "physically valid", or fabrication-ready. Read `drc_evidence.summary.passed` and `.clean` separately. |
| `single_invocation` | accepted external route verification `drc_comparability` | One live DRC invocation produced the count. | Reproducible, stable, differential, or calibrated evidence. |
