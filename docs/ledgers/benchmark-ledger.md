...
| Field | Recorded evidence |
|---|---|
| Question and pre-code prediction | Before implementing the paired slice, a third-pass probe asked whether removing exactly the two measured first gates from B-114 — `Edge.Cuts` curves and `.Cu` `gr_text` — lets any of the six named candidates convert. Its result fixes the prediction for a hypothetical implementation at **0/6 newly converting, so the aggregate ten-board corpus would remain 0 → 0/10**. This is a blocker census, not an implementation result. |
| Method | Read-only third-pass probes over the six B-114 candidates that reached the two gates. The masks were applied in memory only: no transformed board was written and no board bytes were committed. The M.2 and Amalthea probes were initially over-broad; both were corrected to exact layer-filtered masks and rerun. Their outcomes were unchanged. |
...
| Claim | On the six B-114 candidates probed, removing exactly the Edge.Cuts-curve and `.Cu`-text refusals converts **0/6**; together with the other four boards' recorded first-pass refusals, the ten-board corpus remains **0/10**. The next first refusals are listed above, and the corrected exact reruns for M.2 and Amalthea do not change that finding. |
| Non-claims | Not a claim about IPC/GUI text expansion, DRC, routing, placement, fabrication, performance, or the remaining unprobed blockers. Not a claim about a broader board population. No board was written and no private board bytes are committed. |
