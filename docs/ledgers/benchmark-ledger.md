...
| Mutation evidence | `.venv/bin/python scripts/mutation_harness.py docs/mutants/2026-08-13-pad-fabrication-property.json --report report.json` — **19 mutants, 19 killed, 0 survivors, 0 `not_run`**, Python 3.12.13 on macOS-26.5.2-arm64, `baseline_returncode: 0`, spec `sha256:ba88a9e212fe180c7d41f715c7068b641be9372d1bd9b084cb3aa667d4e2e2df`. The mutant→killing-test mapping is in the committed spec, one anchor per mutant, each matching exactly once. Read the mapping and not the count: PP01 (admitting `pad_prop_castellated`) is killed **only** by the partition test, because the value is refused by its own named branch either way — the same shape ADR-0091's surviving mutant took. **Two mutants went `stale_anchor` on a first run and were re-anchored and re-run rather than edited green**, which is the ADR-0098 rule working: adding the disclosure counter changed the call site both had anchored on, and the harness failed the run loudly instead of skipping them. PP18 and PP19 cover the counter itself — never incrementing, and incrementing before the aperture skip so a stencil opening is counted as converted copper. |
