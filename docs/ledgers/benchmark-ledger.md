...
|---|---|
| Run ID | `PENDING` |
| Date and commit | 2026-08-06 UTC; source commit `PENDING`; bound runner `PENDING`; bound adapter `PENDING`. Both digests are recorded inside the artifact's own `configuration`, so a reader never has to guess which revision produced a number. |
