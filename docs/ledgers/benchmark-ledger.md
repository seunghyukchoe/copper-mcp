...
| Method | The subject row **replays B-088 rather than re-measuring it**: the same corpus runner, both grid policies, deterministic replay asserted across repetitions, and a test asserts the row equals B-088's recorded configuration outcome for outcome. What is new is the comparison frame — a frozen baseline roster, a typed resolution rule, and a metric set defined once so a baseline added later lands comparable rather than adjacent. A baseline is `measured` only when **every** declared precondition is *observed* to hold; a precondition this harness cannot observe is never optimistically assumed. |
