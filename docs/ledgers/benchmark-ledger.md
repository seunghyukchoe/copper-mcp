...
| Result | Re-previewed at the finest step their own geometry admits, with the node budget at its ceiling: **0 of 18 routed.** 13 refuse `no_path` — "the start pad cannot contain the routed width" — because on that board the `Edge.Cuts` outline bounds a rectangle that does not contain these pads at all; 5 refuse `grid_budget_exceeded`, needing between 1.8 × 10⁷ and 3.3 × 10¹⁴ nodes to span the pad-to-pad bounding box alone against a ceiling of 5 × 10⁵. |
