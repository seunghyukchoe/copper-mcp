...
#### B-084 — recorded B-076 double allocation

| Field | Recorded evidence |
|---|---|
| Correction | The number `B-076` names two unrelated entries in this ledger. The first, at the "recorded-link target corrections for B-036 and B-057" sub-entry, is a documentation-navigability record from issue #70. The second, "ordered-layer internal proposal oracle", is a measurement of the internal generalized-stack seam ([ADR-0068](../adr/0068-bounded-ordered-layer-routing.md)). Two branches allocated the number in parallel; because this ledger is organized by topic, the two sub-entries landed nine lines apart in the same section and Git merged both without a conflict. |
| Scope | Neither entry is changed, byte for byte, and nothing is renumbered. Renumbering would rewrite append-only history and break the external citations that already exist: `scripts/check_doc_links.py` names "B-076" in both of its link exemptions (meaning the first entry), D-137 and D-140 cite `B-076` in their records (the second), and B-078 supersedes "the unrecorded multilayer differential claim made alongside B-076" (also the second). Read an unqualified `B-076` as ambiguous and cite the sub-entry title alongside it. |
| Detection | Recorded on 2026-08-06 under issues #82 and #83. `scripts/check_ledgers.py` now parses this ledger's `B-` headings and rejects a repeated number. This collision is carried in its closed `RECORDED_COLLISIONS` list, keyed to this entry, so it is reported on every run; the nine legitimate replay sub-entries are carried in a separate closed list and must still be `####` headings beneath the `###` entry they replay. A new duplicate fails the build. |
| Interpretation | This is a bookkeeping record with no measurement, run, artifact, capability, KiCad, electrical, fabrication, or hardware claim. It exists so that a reader who follows a `B-076` citation learns that the number is ambiguous, without the ledger's history being edited to hide that it was. |
