...
### B-090 — KiCad 10.0.5 chamfered- and circular-courtyard oracle parity
...
| Configuration | `kicad-cli pcb drc --format json --units mm --severity-all`, zero-clearance courtyard default; the legalizer at `COURTYARD_CACHE_INSET_NM = 5,000` and `COURTYARD_COLLISION_THRESHOLD_NM = 10,000`, with the circle-side witness requirement doubled per ADR-0077. KiCad's verdict is the presence of a `courtyards_overlap` violation specifically. |
