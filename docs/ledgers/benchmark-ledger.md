...
| Measured probe result | **0/6 newly convert; the aggregate corpus remains 0 → 0/10.** The first refusal moved to the following blockers: TinyTapeout — non-copper `gr_text.layer` invalid arity; LPDDR4 — `setup.stackup`; M.2 — unsupported `kicad_pcb.setup` semantic field; ov9281, Amalthea, and Cynthion — root dimensions. The other four B-114 candidates remain represented by their recorded first-pass gates and are not silently counted as converted. No paired implementation was attempted, so this row does not mislabel the probe as a code before/after differential. |
