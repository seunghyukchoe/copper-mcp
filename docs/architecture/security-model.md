...
| Routing job ledger/worker → local SQLite | Board/prompt leakage, stale-worker overwrite, guessed or expired handles, unbounded retention | Closed redacted records only, source/snapshot and candidate CAS binding, content-addressed idempotent specs, SQLite transaction plus `job_id`/revision update guard, bounded payload/count/TTL, single-worker CAS lease, cooperative cancellation, stale-lease recovery, uniform unknown/expired errors; no request/result repository, remote auth, candidate export, or MCP Tasks claim |
