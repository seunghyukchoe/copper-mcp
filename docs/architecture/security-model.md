...
| Routing job ledger → local SQLite | Board/prompt leakage, stale-worker overwrite, guessed or expired handles, unbounded retention | Closed redacted records only, source/snapshot and candidate CAS binding, content-addressed idempotent specs, SQLite transaction plus `job_id`/revision update guard, bounded payload/count/TTL, cooperative cancellation, uniform unknown/expired errors; no worker, remote auth, candidate export, or MCP Tasks claim |
