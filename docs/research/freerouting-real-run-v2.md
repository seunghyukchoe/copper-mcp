...
That SES was imported through KiCad's GUI into a disposable copy and saved. The harness then ran
the same KiCad-CLI JSON DRC gate for that imported board and for the competing CopperMCP
preview-plus-pure-apply runner result.
...
process timing, and confirms that source bytes remained unchanged. The FreeRouting import and
CopperMCP runner receipts are deliberately self-attested; the harness consequently sets
