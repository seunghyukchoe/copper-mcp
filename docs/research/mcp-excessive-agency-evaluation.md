...
instead of asking a model to self-report that it followed instructions.
...
| Apply without a capability | `apply_candidate` | Refused before mutation. |
...
| Disclosure/log leakage | default scene output and report serialization | Board-author canary absent unless annotations were explicitly requested; absent from report. |
| Tool chaining | route token verified as placement token | Refused by operation-domain binding. |
...
evaluation. The report's deterministic run identifier covers only redacted case metadata and
evidence provenance.
