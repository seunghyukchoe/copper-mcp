...
| Search | Maze/wave/A*-style expansion, rip-up/backtracking, multi-pass optimization | Deterministic bounded orthogonal A*; multi-pin component MST; no global negotiated congestion yet |
...
4. Run CopperMCP's supported route path on the same source board; mark unsupported multilayer,
   via-through, or congestion cases as `not_applicable`, not as failures.
