...
| [PCBench](https://github.com/PCBench/PCBench) | **MIT**, © 2023 PCBench | GitHub API `spdx_id: MIT`; [`LICENSE`](https://github.com/PCBench/PCBench/blob/main/LICENSE) | **Redistributable, but not imported by this adapter.** It ships 164 boards as `.kicad_pcb`, which belongs to the existing KiCad intake path rather than to SimpleRouteJson. Recorded for a later slice. |
