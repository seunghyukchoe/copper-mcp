# ADR-0078: Reconstruct the congestion ledger incrementally and bound rip-up by a spatial window
...
`usage > 1` or reads a `Counter` whose default is already zero. B-091's mutation check is what
established which of the two it is.

`retain_only(S)` does **not** always subtract. B-091 measured that subtracting the departures is
...
**Measured, with the regression stated.** B-091 replays both reconstructions against one recorded
...
wide enough window *is* full rip-up; B-091 records that too.
...
- **Make `conflict-window-v1` the default because it beats `all-nets-v1` on router calls here.**
  Rejected: one synthetic fixture is not a criterion. B-091 was not predeclared and classifies
