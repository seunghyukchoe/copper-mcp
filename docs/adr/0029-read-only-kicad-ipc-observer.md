...
- Related: ADR-0002, ADR-0005, ADR-0022, SEC-023, B-008
...
board serialization plus bounded object counts. The response contains only numeric versions, a
SHA-256 board digest, byte count, object counts, endpoint class, and explicit read-only/source
markers. It never returns the serialized board or board-controlled strings.
...
- CI remains dependency-light because `kicad-python` is optional; deterministic fake-client tests
  cover the complete adapter contract, including endpoint refusal, payload ceilings, and future
  API refusal.
